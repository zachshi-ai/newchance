 ...
 * store.js — 本地优先存储层
 *
 * 供应链决策：不使用任何云端数据库。场馆档案、许可证、人员名册、器材一物一档、日检卡、
 * 开馆单、隐患闭环、变更台账、周期义务 100% 存于浏览器 localStorage；出证物走本地生成的
 ...
 */
import {
  STATE_VERSION, DEFAULT_RENEW_WARN_DAYS, DEFAULT_GEAR_WARN_DAYS, DEFAULT_DUTY_WARN_DAYS,
} from './core.js';

const KEY = 'venuepass.v1';
...
export function emptyState() {
  return {
    ...
    venue: {
      ...
      projects: [],            // ['ski'|'dive'|'climb']
      minStaffPerDay: 0,       // 许可核定的「规定数量」最低在岗持证人数
      address: '', manager: '', phone: '', note: '',
    },
    staff: [],       // { id, name, role, certName, certNo, certValidISO, active, note }
    gear: [],        // { id, name, code, cycleMonths, lastCheckISO, dueISO, outISO, backISO, note }
    daychecks: [],   // { id, dateISO, items: [{key,label,ok,note}], status, note }
    openings: [],    // { id, dateISO, staffIds, staffNames, note, snapshot }
    ...
    settings: {
      renewWarnDays: DEFAULT_RENEW_WARN_DAYS,  // 许可证临期提醒（默认 30=法定届满 30 日续期窗口）
      gearWarnDays: DEFAULT_GEAR_WARN_DAYS,    // 器材检验临期提醒（默认 30 天）
